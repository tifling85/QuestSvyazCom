...
добавленные модули:
CalcServAsyncSelect.py - попытка переписать модуль исспользуя асинхронность с помощью метода select. Начал писать, потом прочел, что все цивилизованные люди используют для этого фреймворк AsyncIO, прекратил писать. Реализована самая основа - сервер поднимается, запускается событийный цикл, обрабатывает селектом файлодескрипторы(сокеты), проверяя их на чтение/запись. При обнаружении события передается в соответствующую функцию

CalcServAsyncIODB.py - переписанный сервер с добавлением асинхронности с помощью фреймворка AsyncIO. Утрачена многопоточность

CalcFuncAsyncIODB.py - переписаны функции под AsyncIO, теперь они корутины

UPDATE 29.09.2019
В асинхронном сервере сделал асинхронными запросы к БД(библиотека aiomysql)

CalcServAsyncThr.py
CalcFuncAsyncThr.py
попробовал переписать сервер, добавив потоки к асинхронности. Вроде полчилось, циклы событий раскидываются по потокам, но сломался таймаут, пока не починил.
асинхронности к БД пока еще тоже нет.
